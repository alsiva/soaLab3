...
const fieldsToCreateDragons = ["name", "age", "coordinate_x", "coordinate_y", "creationdate", "wingspan", "speaking", "color", "eyes_count"]

const fieldToCreateWithType = [
    {name: "name", func: "text"},
    {name : "age", func: (f)=> {parseInt(f)}},
    {name : "coordinate_x", func: "number"},
    {name : "coordinate_y", func: "number"},
    {name : "creationdate", func: "date"},
    {name : "wingspan", func: "number"},
    {name : "speaking", func: "string"},
    {name : "", type: ""},
]

function validateDragonFields(dragonObjToCreate) {
    for(const f of fieldsToCreateDragons){
        let curF = dragonObjToCreate[f];
        if(curF === undefined || curF === ''){
            throw new Error(`поле: ${f} не должно быть пустым`)
        }

    }
}

export {Dragon, createDragonFromJsonObject, prepareJsonDragonXmlConversion, validateDragonFields};